...
- **Directie view**: selecting a Vraagpost shows a form to answer it — free text plus one or more receipt images and/or an invoice PDF. The invoice PDF field reuses the same drag-and-drop dropzone (`components/invoice-dropzone.tsx`) as the rest of the app. The receipt-image field uses `components/vraagposten/receipt-photo-input.tsx`, which pairs that dropzone (gallery/file picker, multi-select) with a separate camera-icon button that opens the device camera directly via a second file input with `capture="environment"` — the camera button only renders below the `sm` breakpoint, since desktop devices rarely have a usable camera. Each newly picked batch of images is appended to the ones already added, and each preview thumbnail has a small X button (top-right) to remove just that image before submitting. Submitting briefly flashes the button green with a checkmark and shows a "Antwoord verzonden" toast (both plain CSS, no animation library), since the submit itself gives no other feedback that it went through. The detail card itself then plays a `framer-motion`-driven transition (`components/vraagposten/vraagpost-action-transition.tsx`, `variant="confirm"`): a checkmark overlay holds briefly, then the card slides right and fades out, after which the detail pane auto-advances to the next open Vraagpost (`lib/vraagpost-queue.ts`'s `findNextOpenVraagpost()`). The same animation plays for finance's **Bevestigen** below, but its completion removes the Vraagpost from finance's list instead of advancing to the next one.
- **Finance view**: selecting the same Vraagpost shows whatever Directie submitted — the note, a clickable image thumbnail per receipt image (each opens a full-size lightbox preview via `components/ui/image-preview-modal.tsx` and has its own download link), and, for the invoice PDF, an inline `<iframe>` preview plus a download link and a **"Converteer naar XML"** button. That button opens `components/vraagposten/vraagpost-xml-convert-modal.tsx`, which runs the *same* PDF→UBL pipeline as the standalone `/pdf-invoice` converter against the already-attached file — no separate extraction logic, no re-upload. Both now share `lib/use-pdf-extraction.ts` (POST to `/api/extract-pdf`, track loading/review/error) so there's exactly one place that talks to the extractor. The modal reuses `components/invoice-edit-form.tsx` as-is for the review screen and the "Download UBL XML" button; its "Opnieuw beginnen" button just closes the modal here, since re-extracting the same file would produce the same result. Like `/pdf-invoice`, the button is disabled in the static-export build (`NEXT_PUBLIC_STATIC_EXPORT`) — there's no server there to run the Python extractor. Once Directie has answered, two actions appear next to the status chip:
...
| `lib/vraagpost-answers.ts` | `Answer` type (client-only mutable state; `receiptImages: File[]` for multiple receipt images) |
...
| `lib/use-object-url.ts` | `useObjectUrl` / `useObjectUrls` — create/revoke `blob:` URL(s) for one `File` or a `File[]` |
...
| `components/vraagposten/vraagpost-answer-form.tsx` | Directie's note + receipt image(s) + invoice PDF form; supports adding multiple receipt images and removing one at a time via an X button on its thumbnail; shows finance's note when the Vraagpost was reopened |
| `components/vraagposten/receipt-photo-input.tsx` | Receipt image picker: gallery dropzone (multi-select) + mobile-only camera-capture button |
...
| `components/vraagposten/vraagpost-answer-view.tsx` | Finance's read-only view of a submitted answer, including the invoice PDF preview and "Converteer naar XML" button |
| `components/vraagposten/vraagpost-xml-convert-modal.tsx` | Modal that extracts finance's attached invoice PDF and reuses `InvoiceEditForm` for review + XML download |
| `lib/use-pdf-extraction.ts` | Shared hook wrapping `POST /api/extract-pdf` and its loading/review/error states — used by both `/pdf-invoice` and the modal above |
...
| `components/ui/image-preview-modal.tsx` | Full-size lightbox for a receipt image, opened by clicking its thumbnail in the Finance view |
