...
- **Finance view**: selecting the same Vraagpost shows whatever Directie submitted — the note, a clickable image thumbnail per receipt image (each opens a full-size lightbox preview via `components/ui/image-preview-modal.tsx` and has its own download link), and a download link for the PDF. Once Directie has answered, two actions appear next to the status chip:
